...
DB_RESTORE_REMOTE_PATH="/nas/content/live/asianlegacylib/wp-content/_db_backups/nas/content/live/asianlegacystg/wp-content/_db_backups"
...
# Pass sql.tar.bz2 file name with a stage SQL dump file name in it
# Check if a required argument was provided
if [ "$#" -ne 1 ]; then
    echo "Error. Usage: $0 /path/to/your/stage-backup-file.sql.tar.bz2"
    exit 1
fi

# The first required argument is the stage-backup-file.sql.tar.bz2 archive file name
FILE_PATH="$1"
...
if [ -f "${DB_BACKUP_PATH}/${FILE_PATH}" ]; then
    ...
    if [ -f "${DB_RESTORE_REMOTE_PATH}/${STRIPPED_NAME}" ]; then
        ...
        printf "Fix stage to prod website URLs...\n"
        wp db query < "${DB_RESTORE_REMOTE_PATH}/_update_stage_to_prod_option_urls.sql"
